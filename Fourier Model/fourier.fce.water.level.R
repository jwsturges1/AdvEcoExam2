...
fce.srs3.level.data<-read.table("fce.srs3.level.csv", sep=",", header=TRUE)[,1:2]
...
fce.srs3.level.data<-asStreamflow(fce.srs3.level.data,start.date="2000-01-01",end.date="2021-12-31", river.name="SRS-3")

###Plot data to visually inspect time-series pulses
fce.srs3.level<-fce.srs3.level.data$data
plot(fce.srs3.level$discharge~fce.srs3.level$date, type="l", ylab="Water Level (NAVD88 + 42 (cm)", xlab="Date")
